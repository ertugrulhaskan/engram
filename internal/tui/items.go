...
func (m Model) docItems() []Item {
	...
	for _, d := range m.docs {
		// Badge names the provider for non-Claude rules, so the instruction
		// files in one project group don't all read "rules". They share a single
		// colour — the colour says "someone else's rules", the label says whose,
		// so adding a vendor costs a word and not another hue.
		badge, bcolor := "rules", t.TFeedback
		switch {
		case d.Kind == memory.DocIndex:
			badge, bcolor = "index", t.TReference
		case d.Provider != memory.ProviderClaude:
			badge, bcolor = string(d.Provider), t.TProject
		}
