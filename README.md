...
**Latest release: `v0.4.0`** · open source ([MIT](LICENSE)) · website:
...
| `a`        | mark every memory in the list — with a type filter set, that is the whole type. Press again to clear those marks |
...
`team`, `secrets`, `source`), and `internal/tui` is Bubble Tea UI with no file logic. See
